...
intégré comme dans le flight planner HELIX — un seul device GPU partagé entre l'UI
...
vendor/           forks entretenus par HELIX-CORSICA — provenance : vendor/FORKS.md
  wgpui/          GPUI (Zed → gpui-ce → WGPUI) + patchs Helix + patchs GPUI-3D
  gpui-component/ composants (longbridge) adaptés à ce GPUI
  priority-threadpool/  pool de fils (timers GPUI), course corrigée
```

La recette (celle du globe Terra, `helix_gpui_shell::terra_render`) :
...
et `Compat Helix` dans `vendor/wgpui` avant d'y reprendre un correctif amont.
