...
Depuis le 2026-09-24, **HELIX-CORSICA entretient ces forks**. Ils ont divergé de leurs
amonts (patchs Helix, puis patchs GPUI-3D) et ne sont plus synchronisés
...
| Patchs Helix | `HELIX-CORSICA/helix-flight-planner`, `vendor/wgpui` | vendoré en `a98026205` (2026-09-02), copié ici depuis `4751d3d4e` (2026-09-23) |
| Patchs GPUI-3D | ce dépôt | branche `optim` |

Licence : Apache-2.0 (`vendor/wgpui/LICENSE-APACHE`).

**Patchs Helix** (~40 commits du flight planner, 2026-09-02 → 2026-09-23) : API
rétablies pour gpui-component (`src/helix_compat.rs`, blocs « Compat Helix »), device
...
| Adaptation WGPUI + Helix | `HELIX-CORSICA/helix-flight-planner`, `vendor/gpui-component` | fork local `ebef0e1c`, vendoré en `a98026205`, lock `19e1cdf75` |
...
| Patch Helix | `helix-flight-planner` `5e5c4caae` : course `fetch_add`/`push` corrigée (jobs orphelins ⇒ timers GPUI perdus) | — |
...
3. Rechercher `GPUI-3D` et `Compat Helix` dans les fichiers touchés pour ne pas écraser
   un patch.
4. Relancer la suite du fork et le banc (`README.md`, section Performance) : parité et
   mesures.
5. Commiter en citant le commit amont.

Le flight planner garde sa propre copie : un correctif utile aux deux se reporte dans
chaque dépôt.
