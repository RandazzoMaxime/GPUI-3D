...
pub(crate) struct EntityMap {
    ...
    /// depuis sa dernière lecture sans la comparer (helix `entity_watch`, 0,3 ms par tick évités).
...
impl EntityMap {
    ...
    pub fn lease<T>(&mut self, pointer: &Entity<T>) -> Lease<T> {
        ...
        *self.mutations.entry(pointer.entity_id).unwrap().or_insert(0) += 1;
    ...
    /// Prêts mutables cumulés sur cette entité (voir `mutations`).
    pub fn mutation_count(&self, entity_id: EntityId) -> u64 {
        self.mutations.get(entity_id).copied().unwrap_or(0)
    }
    ...
    pub unsafe fn lease_unchecked<T: 'static>(&mut self, entity_id: EntityId) -> LeaseUnchecked<T> {
        ...
        *self.mutations.entry(entity_id).unwrap().or_insert(0) += 1;
