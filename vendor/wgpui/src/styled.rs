...
pub trait Styled: Sized {
    ...
    /// Compat Helix (voir src/helix_compat.rs) : alias de [`Styled::flex_grow`].
    fn flex_grow_1(self) -> Self {
        self.flex_grow()
    }

    /// Compat Helix (voir src/helix_compat.rs) : alias de [`Styled::flex_shrink`].
    fn flex_shrink_1(self) -> Self {
        self.flex_shrink()
    }

    /// Compat Helix (voir src/helix_compat.rs) : taille minimale = 100 %
    /// du parent, sur les deux axes.
    fn min_size_full(mut self) -> Self {
        self.style().min_size.width = Some(crate::relative(1.).into());
        self.style().min_size.height = Some(crate::relative(1.).into());
        self
    }
