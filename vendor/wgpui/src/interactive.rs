...
impl ClickEvent {
    ...
    /// Compat Helix (voir src/helix_compat.rs) : clic du bouton du milieu.
    ///
    /// `Keyboard`: false
...
impl ScrollDelta {
    ...
    /// Pixels for one `Lines(1.0)` unit on overflow/list scroll.
    ///
    ...
    /// maps a default notch to 100px; Helix uses 55px (UgCS UITK ~54px).
    pub const OVERFLOW_LINE_PX: f32 = 55.0;

    /// Line-delta → pixels for panel/list overflow. Trackpad `Pixels` pass
    /// through unchanged.
    pub fn overflow_pixel_delta(&self) -> Point<Pixels> {
        self.pixel_delta(px(Self::OVERFLOW_LINE_PX))
    }
