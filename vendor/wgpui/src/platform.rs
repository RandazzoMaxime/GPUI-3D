...
pub(crate) struct RequestFrameOptions {
    /// Une surface wgpu a publié une trame que le compositeur n'a pas encore
    /// promue en `display` : la présenter, même sans vue sale.
...
pub(crate) trait PlatformWindow: HasWindowHandle + HasDisplayHandle {
    ...
    /// Prédicat « cette fenêtre a quelque chose à montrer », interrogé hors
    /// dessin par la boucle d'événements.
    fn on_wants_frame(&self, _callback: Box<dyn FnMut() -> bool>) {}
    ...
    /// `false` si la plateforme ou la surface ne le permet pas.
    fn set_present_mode(&self, _mode: crate::WindowPresentMode) -> bool {
        false
    }

    /// `key == 0` recopie le framebuffer à chaque filtre. `key > 0` fige la
    /// copie tant que `key` ne change pas (modale ouverte : l'UI derrière
    /// ne bouge pas).
    fn lock_glass_backdrop(&self, _key: u32) {}
