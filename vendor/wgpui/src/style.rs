...
pub enum Filter {
    ...
    Blur(Pixels),
    /// Compat Helix (voir src/helix_compat.rs) : saturation, `1.0` = neutre.
    /// Maps to CSS `saturate(<f32>)`. Accepté mais PAS ENCORE RENDU : les
    /// passes de flou l'ignorent, à câbler dans le shader de composition.
    Saturate(f32),
}
