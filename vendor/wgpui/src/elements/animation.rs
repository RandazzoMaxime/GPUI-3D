...
// Compat Helix (voir src/helix_compat.rs) : transfère les enfants à l'élément
// animé, pour que `.child(..)` et les extensions bornées sur `ParentElement`
// restent utilisables après `.with_animation(..)`.
impl<E: crate::ParentElement + IntoElement + 'static> crate::ParentElement for AnimationElement<E> {
