...
fn fs_backdrop_filter(input: BackdropFilterVarying) -> @location(0) vec4<f32> {
    ...
    if blur_radius < 0.5 {
        ...
        // Step the kernel: a 24px radius used to sample ±48 in both axes
        // (≈9400 taps/pixel) and stalled large modal sheets. Stride radius/4
        // keeps the same support with ~81 taps.
        let max_blur_radius = 32.0;
        let effective_blur_radius = min(blur_radius, max_blur_radius);
        let step = max(effective_blur_radius / 4.0, 1.0);
        let kernel_size = i32(ceil(effective_blur_radius / step));
