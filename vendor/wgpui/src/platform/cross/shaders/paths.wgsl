...
//   0*0 = 0 < 1  →  always kept.
//
// Antialiased-stroke triangles (Helix, profile chart) use t <= -1: s is the
...
fn fs_path(v: PathVarying) -> @location(0) vec4<f32> {
    let s = v.st.x;
    let t = v.st.y;
    let s_per_px = max(length(vec2<f32>(dpdx(s), dpdy(s))), 1e-6);
    ...
    var coverage = 1.0;
    if t <= -1.0 {
        let half_width = (-1.0 - t) / s_per_px;
        let d = s / s_per_px;
        coverage = clamp(min(d + 0.5, half_width) - max(d - 0.5, -half_width), 0.0, 1.0);
        if coverage <= 0.0 {
            discard;
        }
    } else if s * s > t {
