...
// GPUI-3D : glyphe compact (88 octets). Couleur unie et transformation identité dans
// le cas courant ; dégradé et transformation vivent dans `b_sprite_extras[extra]`.
...
@group(3) @binding(1) var<storage, read> b_sprite_extras: array<SpriteExtra>;
...
fn vs_mono_sprite(@builtin(vertex_index) vertex_id: u32, @builtin(instance_index) instance_id: u32) -> MonoSpriteVarying {
    ...
    var transformation = TransformationMatrix(mat2x2<f32>(1.0, 0.0, 0.0, 1.0), vec2<f32>(0.0, 0.0));
    var text_color: TextColor;
    text_color.tag = 0u;
    text_color.solid = sprite.color;
    if (sprite.extra != 0xffffffffu) {
        let extra = b_sprite_extras[sprite.extra];
        transformation = extra.transformation;
        text_color = extra.text_color;
    }

    var out = MonoSpriteVarying();
    out.position = to_device_position_transformed(unit_vertex, sprite.bounds, transformation);
    ...
    out.color = text_gradient_color(text_color, pixel_position, sprite.content_mask);

    out.clip_distances = distance_from_clip_rect_transformed(unit_vertex, sprite.bounds, sprite.content_mask, transformation);
    return out;
}
