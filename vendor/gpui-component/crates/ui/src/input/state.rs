...
pub(super) fn sync_focused_input_registry(
    ...
    Root::try_update(window, cx, |root, _, cx| {
        // Appele au rendu de CHAQUE champ, a chaque trame : notifier `Root`
        // sans changement de registre invalidait toute vue cachee au-dessus
        // d'un champ de saisie, a toutes les trames.
        let next = if focused {
            Some(state.clone())
