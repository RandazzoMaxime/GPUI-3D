...
impl<M: InputModeKind> Element for TextElement<M> {
    ...
    fn paint(
        ...
        self.state.update(cx, |state, cx| {
            // Ce `notify` etait inconditionnel : chaque champ de saisie peint
            // redemandait une trame, donc l'application ne s'immobilisait
            // jamais et aucune vue cachee au-dessus d'un champ ne pouvait
            // etre reutilisee (elle voyait sa dependance invalidee a chaque
            // trame). Il ne se declenche plus que si l'etat enregistre change.
