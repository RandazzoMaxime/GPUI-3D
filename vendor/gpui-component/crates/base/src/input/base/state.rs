...
impl<M: InputModeKind> InputBaseState<M> {
    ...
    pub(super) fn on_scroll_wheel(
        ...
        if self.is_single_line() && delta.y.abs() >= delta.x.abs() {
            return;
        }
    ...
    pub(super) fn update_scroll_offset(
        ...
        let moved = self.scroll_handle.offset() != offset;
        self.scroll_handle.set_offset(offset);
        // Appele depuis le paint du champ a chaque trame : notifier sans
        // mouvement reel reprogrammait une trame indefiniment.
        if moved {
            cx.notify();
        }
    }
